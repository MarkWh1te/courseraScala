...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = w.map(_.toLower).groupBy(x=>x).map(x=>(x._1,x._2.length)).toList.sortWith(_._1<_._1)

  /** Converts a sentence into its character occurrence list. */
  def sentenceOccurrences(s: Sentence): Occurrences = s.flatMap(x=>x.map(_.toLower)).groupBy(x=>x).map(x=>(x._1,x._2.length)).toList.sortWith(_._1<_._1)
  ...
  // TODO: find out what is wrong with the code below
  // def combinations(occurrences: Occurrences): List[Occurrences] =
  //   occurrences match {
  //     case List() => List(List())
  //     case head :: tail => {
  //       val xsresults = combinations(tail)
  //       xsresults ++ (
  //         for {
  //           xsresult <- xsresults
  //           num <- 1 to head._2
  //         } yield (head._1,num) :: xsresults
  //       )
  //     }
  //   }
  def combinations(occurrences: Occurrences): List[Occurrences] =
